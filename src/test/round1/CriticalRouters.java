...
public class CriticalRouters {
    ...
    static List<Integer> getCriticalNodes(int[][] links, int numLinks, int numRouters) {
        time = 0;
    ...
    private static void dfs(Map<Integer, Set<Integer>> map,
                            int[] low, int[] ids, int[] parent,
                            int cur, Set<Integer> res) {
        int children = 0;
        ...
        for (int nei : map.get (cur)) {
            if (ids[nei] == -1) {
                ...
                if (parent[cur] == -1 && children > 1 || (parent[cur] != 1 - 1 && low[nei] >= ids[cur]))
                    res.add (cur);
